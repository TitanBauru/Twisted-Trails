...
uniform vec4 u_params3; // normalDistance, diffuse, specular, reflection
...
void main() {
	...
	// Diffuse
	//vec3 diffuse = lightAttenuation * ((v_vColour.rgb*u_params2.x) + ((kDiffuse*(albedo/M_PI)+specularCT)*v_vColour.rgb*u_params3.y));
	vec3 diffuse = u_params2.x * lightAttenuation * ((v_vColour.rgb*u_params3.y) + (specularCT*v_vColour.rgb*u_params3.z)); // looks better..
	
	// Reflections
	vec2 reflectionUV = v_vTexcoord + (N.xy * roughness * u_params3.w);
