...
uniform vec4 u_params2; // normalDistance, diffuse, specular, reflection
...
void main() {
	...
	// Diffuse
	//vec3 diffuse = u_params.z * lightAttenuation * ((v_vColour.rgb*u_params2.y) + ((kDiffuse*(albedo/M_PI)+specularCT)*v_vColour.rgb*u_params2.z));
	vec3 diffuse = u_params.z * lightAttenuation * ((v_vColour.rgb*u_params2.y) + (specularCT*v_vColour.rgb*u_params2.z)); // looks better..
	
	// Reflections
	vec2 reflectionUV = v_vTexcoord + (N.xy * roughness * u_params2.w);
