...
function __Crystal_DebugInterfaces() constructor {
	...
	static PanelClassCrystalRenderer = function() {
		...
		var _lightsMenu = _inspector.AddElement(new __Crystal_UIElementSection("Lights", false, 2), _menuA);
			...
			_inspector.AddElement(new __Crystal_UIElementSlider(_ref, "__lightsCompensation", "Lights Compensation", 0, 5), _lightsMenu);
			_inspector.AddElement(new __Crystal_UIElementText("Lights Blendmode:"), _lightsMenu);
			_inspector.AddElement(new __Crystal_UIElementRadio(_ref, "__lightsBlendMode", ["Multiply", "Multiply Reinhard LDR", "Multiply Normalized", "Add"]), _lightsMenu);
			// LIGHTS EDITOR
			var _lightsEditorMenu = _inspector.AddElement(new __Crystal_UIElementSection("Lights Editor", false, 2), _lightsMenu);
				...
				_inspector.AddElement(new __Crystal_UIElementCheckbox(self, "__editorLightEditing", "Enable Editing"), _lightsEditorMenu);
			...
			var _lightsCollisionMenu = _inspector.AddElement(new __Crystal_UIElementSection("Collision", false, 2), _lightsMenu);
				...
				_inspector.AddElement(new __Crystal_UIElementCustom(, function(_element, _ui) {
					if (__panelCurrentClass.__isGeneratingLightsCollision) __panelInspector.__bake();
					draw_set_color(__panelCurrentClass.GetLightsCollisionAt(__mouseX, __mouseY));
					draw_circle(_element.xx+8, _element.yy+8, 8, false);
					draw_set_color(c_white);
					_element.height = 16;
				}), _lightsCollisionMenu);
		...
		var _materialsMenu = _inspector.AddElement(new __Crystal_UIElementSection("Materials", false, 2), _menuA);
			...
			// list
			materialsList = []; // list of structs
			materialsUiList = [];
			materialsUiListSelected = 0;
			
			var _materialsListMenu = _inspector.AddElement(new __Crystal_UIElementSection("List", false, 2), _materialsMenu);
				...
				_inspector.AddElement(new __Crystal_UIElementCustom(function(_element) {
					...
					__panelInspector.__bake(); // make sure inspector continues drawing every frame
					ds_list_add(__overlayRenderables, function() {
						// visualize current material position
						if (__editorSelectedMaterial != noone) {
							var _owner = __editorSelectedMaterial.__owner;
							if (_owner != noone) {
								draw_set_color(c_lime);
								draw_set_circle_precision(4);
								draw_circle(_owner.x, _owner.y, 8, true);
				...
				_inspector.AddElement(new __Crystal_UIElementText("Show UI Inspector first."), _materialsListMenu);
				_inspector.AddElement(new __Crystal_UIElementButton("Update List", function() {
					array_resize(materialsList, 0);
					array_resize(materialsUiList, 0);
					__editorSelectedMaterialWeakRef = undefined;
					materialsUiListSelected = -1;
					var _materials = [__panelCurrentClass.__matNormalSprites,
										__panelCurrentClass.__matEmissiveSprites,
										__panelCurrentClass.__matReflectionSprites,
										__panelCurrentClass.__matMetallicSprites,
										__panelCurrentClass.__matRoughnessSprites,
										__panelCurrentClass.__matAoSprites,
										__panelCurrentClass.__matMaskSprites];
					var msize = array_length(_materials);
					for (var m = 0; m < msize; ++m) {
						var _matSprList = _materials[m];
						//show_debug_message($"{_matSprList}, {ds_list_size(_matSprList)}");
						// for each item inside list, add non-duplicated (because we need to unify)
						for (var l = 0; l < ds_list_size(_matSprList); ++l) {
							var _matStruct = _matSprList[| l];
							if (!array_contains(materialsList, _matStruct)) {
								array_push(materialsList, _matStruct);
							}
						}
					}
					// add items to radio UI with custom text
					var i = 0, isize = array_length(materialsList);
					repeat(isize) {
						var _matStruct = materialsList[i];
						var _id = _matStruct.__owner;
						array_push(materialsUiList, $"{object_get_name(_id.object_index)} ({string(_id ?? "N/A owner")})");
						++i;
					}
				...
				_inspector.AddElement(new __Crystal_UIElementButton("Open Selected", function() {
					if (materialsUiListSelected == -1) return;
					// get current selected struct from material list
					if (array_length(materialsList) > 0) {
						__editorSelectedMaterial = materialsList[materialsUiListSelected];
						__editorSelectedMaterialWeakRef = weak_ref_create(__editorSelectedMaterial);
					
						// open material sprite inspector (the inspector will already know the material struct, defined above)
						if (__inspector != undefined) {
							method(self, __interfaces.InspectorMaterialSprite)();
						}
					}
				}), _materialsListMenu);
				
				_inspector.AddElement(new __Crystal_UIElementRadio(self, "materialsUiListSelected", materialsUiList), _materialsListMenu);
				
				
		// > Culling
		var _cullingMenu = _inspector.AddElement(new __Crystal_UIElementSection("Culling", false, 2), _menuA);
			_inspector.AddElement(new __Crystal_UIElementText("Culling disables Lights, Shadows and Materials outside of the camera's view when the camera is moving (or idle, using the auto update timer). Note that you can still control the disabling of lights and materials individually."), _cullingMenu);
	...
	static PanelClassMaterialLayer = function() {
		...
		_inspector.AddElement(new __Crystal_UIElementText("Layer Range List:"), _menuA);
	...
	static PanelAux = function() {
		...
		_inspector.AddElement(new __Crystal_UIElementCheckbox(self, "__editorIsEnabled", "Show UI Inspector"), _auxMenuA);
	...
	static InspectorBasicLight = function() {
		...
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "image_index",, -1, 10, 1), _menuA);
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "image_speed",, 0, 2), _menuA);
	...
	static InspectorPointLight = function() {
		...
		_inspector.AddElement(new __Crystal_UIElementCheckbox(__editorSelectedInstance, "enabled"), _menuA);
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "depth",, -15000, 15000, 1), _menuA);
		var _shaderMenu = _inspector.AddElement(new __Crystal_UIElementSection("Shader", false, 2), _menuA);
			_inspector.AddElement(new __Crystal_UIElementText("shaderType"), _shaderMenu);
			_inspector.AddElement(new __Crystal_UIElementRadio(__editorSelectedInstance, "shaderType", ["Basic", "Phong", "BRDF"]), _shaderMenu);
			_inspector.AddElement(new __Crystal_UIElementText("shadowShaderType"), _shaderMenu);
			_inspector.AddElement(new __Crystal_UIElementRadio(__editorSelectedInstance, "shadowShaderType", ["Basic", "Smooth"]), _shaderMenu);
		var _colorMenu = _inspector.AddElement(new __Crystal_UIElementSection("Color", false, 2), _menuA);
	...
	static InspectorSpriteLight = function() {
		...
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "image_index",, -1, 10, 1), _menuA);
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "image_speed",, 0, 2), _menuA);
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "image_xscale",, -10, 10), _menuA);
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "image_yscale",, -10, 10), _menuA);
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "image_angle",, 0, 360, 1), _menuA);
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "image_alpha",, 0, 1), _menuA);
		_inspector.AddElement(new __Crystal_UIElementCheckbox(__editorSelectedInstance, "castShadows"), _menuA);
		_inspector.AddElement(new __Crystal_UIElementCheckbox(__editorSelectedInstance, "selfShadows"), _menuA);
	...
	// Material Sprite
	static InspectorMaterialSprite = function() {
		var _inspector = __inspector;
		_inspector.RemoveElements();
		
		var _menuA = _inspector.AddElement(new __Crystal_UIElementSection("Material Sprite", true));
		_inspector.AddElement(new __Crystal_UIElementText(,,,function() {
			if (__editorSelectedMaterial == noone) return "N/A material";
			var _id = __editorSelectedMaterial.__owner;
			if (_id != noone) return $"{object_get_name(_id.object_index)} ({string(_id)})";
			return "AAA";
		}), _menuA);
		
		_inspector.AddElement(new __Crystal_UIElementText(,,,function() {return $"x: {__editorSelectedMaterial.x}, y: {__editorSelectedMaterial.y}";}), _menuA);
		
		_inspector.AddElement(new __Crystal_UIElementCheckbox(__editorSelectedMaterial, "enabled"), _menuA);
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "depth",, -15000, 15000, 1), _menuA);
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "angle",, 0, 360, 1), _menuA);
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "xScale",, -10, 10), _menuA);
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "yScale",, -10, 10), _menuA);
		// Apply
		_inspector.AddElement(new __Crystal_UIElementButton("Apply", function() {
			__editorSelectedMaterial.Apply();
		}), _menuA);
		// Remove
		_inspector.AddElement(new __Crystal_UIElementButton("Remove", function() {
			__editorSelectedMaterial.Destroy();
		}), _menuA);
		
			// normal
			var _menuNormal = _inspector.AddElement(new __Crystal_UIElementSection("Normal", false, 2));
			_inspector.AddElement(new __Crystal_UIElementText("",,, function() {return __editorSelectedMaterial.normalSprite != undefined ? sprite_get_name(__editorSelectedMaterial.normalSprite) : "N/A";}), _menuNormal);
			_inspector.AddElement(new __Crystal_UIElementButton("Select Sprite Asset", function() {
				if (__assetSelector != undefined) __uiSystem.RemoveContainer(__assetSelector);
				__assetSelector = method(self, __interfaces.AssetSelector)( function(_sprite) {__editorSelectedMaterial.normalSprite=_sprite; __inspector.__bake();}, asset_sprite );
			}), _menuNormal);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "normalSpriteSubimg",, -1, 10, 1), _menuNormal);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "normalIntensity",, 0, 1), _menuNormal);
			
			// emissive
			var _menuEmissive = _inspector.AddElement(new __Crystal_UIElementSection("Emissive", false, 2));
			_inspector.AddElement(new __Crystal_UIElementText("",,, function() {return __editorSelectedMaterial.emissiveSprite != undefined ? sprite_get_name(__editorSelectedMaterial.emissiveSprite) : "N/A";}), _menuEmissive);
			_inspector.AddElement(new __Crystal_UIElementButton("Select Sprite Asset", function() {
				if (__assetSelector != undefined) __uiSystem.RemoveContainer(__assetSelector);
				__assetSelector = method(self, __interfaces.AssetSelector)( function(_sprite) {__editorSelectedMaterial.emissiveSprite=_sprite; __inspector.__bake();}, asset_sprite );
			}), _menuEmissive);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "emissiveSpriteSubimg",, -1, 10, 1), _menuEmissive);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "emissiveIntensity",, 0, 1), _menuEmissive);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "emission",, 0, 10), _menuEmissive);
			_inspector.AddElement(new __Crystal_UIElementColor(__editorSelectedMaterial, "emissionColor"), _menuEmissive);
			
			// reflection
			var _menuReflection = _inspector.AddElement(new __Crystal_UIElementSection("Reflection", false, 2));
			_inspector.AddElement(new __Crystal_UIElementText("",,, function() {return __editorSelectedMaterial.reflectionSprite != undefined ? sprite_get_name(__editorSelectedMaterial.reflectionSprite) : "N/A";}), _menuReflection);
			_inspector.AddElement(new __Crystal_UIElementButton("Select Sprite Asset", function() {
				if (__assetSelector != undefined) __uiSystem.RemoveContainer(__assetSelector);
				__assetSelector = method(self, __interfaces.AssetSelector)( function(_sprite) {__editorSelectedMaterial.reflectionSprite=_sprite; __inspector.__bake();}, asset_sprite );
			}), _menuReflection);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "reflectionSpriteSubimg",, -1, 10, 1), _menuReflection);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "reflectionIntensity",, 0, 1), _menuReflection);
			_inspector.AddElement(new __Crystal_UIElementColor(__editorSelectedMaterial, "reflectionColor"), _menuReflection);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "reflectionXscale",, -10, 10), _menuReflection);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "reflectionYscale",, -10, 10), _menuReflection);
			
			// metallic
			var _menuMetallic = _inspector.AddElement(new __Crystal_UIElementSection("Metallic", false, 2));
			_inspector.AddElement(new __Crystal_UIElementText("",,, function() {return __editorSelectedMaterial.metallicSprite != undefined ? sprite_get_name(__editorSelectedMaterial.metallicSprite) : "N/A";}), _menuMetallic);
			_inspector.AddElement(new __Crystal_UIElementButton("Select Sprite Asset", function() {
				if (__assetSelector != undefined) __uiSystem.RemoveContainer(__assetSelector);
				__assetSelector = method(self, __interfaces.AssetSelector)( function(_sprite) {__editorSelectedMaterial.metallicSprite=_sprite; __inspector.__bake();}, asset_sprite );
			}), _menuMetallic);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "metallicSpriteSubimg",, -1, 10, 1), _menuMetallic);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "metallicIntensity",, 0, 1), _menuMetallic);
			
			// roughness
			var _menuRoughness = _inspector.AddElement(new __Crystal_UIElementSection("Roughness", false, 2));
			_inspector.AddElement(new __Crystal_UIElementText("",,, function() {return __editorSelectedMaterial.roughnessSprite != undefined ? sprite_get_name(__editorSelectedMaterial.roughnessSprite) : "N/A";}), _menuRoughness);
			_inspector.AddElement(new __Crystal_UIElementButton("Select Sprite Asset", function() {
				if (__assetSelector != undefined) __uiSystem.RemoveContainer(__assetSelector);
				__assetSelector = method(self, __interfaces.AssetSelector)( function(_sprite) {__editorSelectedMaterial.roughnessSprite=_sprite; __inspector.__bake();}, asset_sprite );
			}), _menuRoughness);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "roughnessSpriteSubimg",, -1, 10, 1), _menuRoughness);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "roughnessIntensity",, 0, 1), _menuRoughness);
			
			// ao
			var _menuAo = _inspector.AddElement(new __Crystal_UIElementSection("Ambient Occlusion", false, 2));
			_inspector.AddElement(new __Crystal_UIElementText("",,, function() {return __editorSelectedMaterial.aoSprite != undefined ? sprite_get_name(__editorSelectedMaterial.aoSprite) : "N/A";}), _menuAo);
			_inspector.AddElement(new __Crystal_UIElementButton("Select Sprite Asset", function() {
				if (__assetSelector != undefined) __uiSystem.RemoveContainer(__assetSelector);
				__assetSelector = method(self, __interfaces.AssetSelector)( function(_sprite) {__editorSelectedMaterial.aoSprite=_sprite; __inspector.__bake();}, asset_sprite );
			}), _menuAo);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "aoSpriteSubimg",, -1, 10, 1), _menuAo);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "aoIntensity",, 0, 1), _menuAo);
			
			// mask
			/*var _menuMask = _inspector.AddElement(new __Crystal_UIElementSection("Mask (TO-DO)", false, 2));
			_inspector.AddElement(new __Crystal_UIElementText("",,, function() {return __editorSelectedMaterial.maskSprite != undefined ? sprite_get_name(__editorSelectedMaterial.maskSprite) : "N/A";}), _menuMask);
			_inspector.AddElement(new __Crystal_UIElementButton("Select Sprite Asset", function() {
				if (__assetSelector != undefined) __uiSystem.RemoveContainer(__assetSelector);
				__assetSelector = method(self, __interfaces.AssetSelector)( function(_sprite) {__editorSelectedMaterial.maskSprite=_sprite; __inspector.__bake();}, asset_sprite );
			}), _menuMask);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "maskSpriteSubimg",, -1, 10, 1), _menuMask);
			_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedMaterial, "maskIntensity",, 0, 1), _menuMask);
			*/
	}
	#endregion
...
function Crystal_DebugUI(_origin=undefined, _classInstance=undefined, _isOpened=false, _startMaximized=true) constructor {
	...
	__editorLightEditing = false;
	__editorIsEnabled = false;
	__editorSelectedInstance = noone;
	__editorSelectedMaterial = noone;
	__editorSelectedMaterialWeakRef = undefined;
	...
	__panelWindow = new __Crystal_UIWindow($"Crystal Lighting Engine {CLE_VERSION}");
	...
	static Draw = function(_font=__cle_fntDebugUI) {
		...
		if (__editorIsEnabled) {
			...
			if (__inspectorWindow == undefined) {
				...
				// >>> Editing Lights
				if (__editorIsEditingLights) {
					// If mouse is OUTSIDE UI (not clicking on an container), you CAN EDIT
					if (__uiSystem.__containerInFocus == undefined) {
						// if lights editing
						if (__editorLightEditing) {
			...
			// >>> Editing Materials
			if (!weak_ref_alive(__editorSelectedMaterialWeakRef) && __editorSelectedMaterialWeakRef != undefined) {
				__inspector.RemoveElements();
				__editorSelectedMaterialWeakRef = undefined;
			}
		} else {
