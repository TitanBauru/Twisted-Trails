...
/// @returns {Struct} 
function Crystal_Shadow(_owner, _shadowType, _transformMode=SHADOW_SOFT_TRANSFORMED) constructor {
	...
	static Apply = function(_renderer=global.__CrystalCurrentRenderer) {
		...
		if (array_length(__frames) == 0) {
			__crystal_trace("Shadow not applied. No meshes added previously", 1);
			return;
		}
	...
	static AddMesh = function(_mesh, _frame=-1) {
		if (_frame < 0) {
			...
			frameCount = array_length(__frames);
		}
		return self;
	}
