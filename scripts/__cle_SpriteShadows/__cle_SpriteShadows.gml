	...
	__step = function() {
		...
		if (__regenerate) {
			...
			if (mode == CRYSTAL_SHADOW_MODE.SPRITE_BAKED) {
				...
				var _batches = __batches;
				repeat(__shadowsAmount) {
					with(_shadowsArray[i]) {
						if (__destroyed) {
							array_delete(_shadowsArray, i, 1);
						} else {
							if (enabled) {
								...
								if (_currentTexture != __spriteTexture) {
									...
									array_push(_batches, {vBuff: _currentBatch, texture: _currentTexture});
...
function Crystal_ShadowSprite(_owner, _sprite, _spriteSubimg) constructor {
