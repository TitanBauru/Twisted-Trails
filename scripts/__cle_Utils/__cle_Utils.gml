...
// -------------------------------------------------
// Dithering
/// @desc Since deferred rendering is notorious for issues with opacity, this should help in some cases where you need to have things with a lot of transparency (like gradients) and WITH depth buffer enabled. The shader discards fragments based on opacity, using dithering.
...
/// @func __crystal_trace(text, level)
/// @param {String} text
/// @ignore
...
/// @ignore
function __crystal_directory_get_contents(_pathSource, _contentsArray, _extension="*.*", _searchFiles=true, _searchFolders=true, _searchSubdir=true) {
	...
	var _i = 0, _fileName = "", _path = "", _dirName = "", _data = undefined, _progress = 0;
	repeat(_filesCount) {
		_fileName = _contents[_i];
		_path = _pathSource + "/" + _fileName; // the path of the content (folder or file)
		_dirName = filename_name(filename_dir(_path));
		_data = undefined;
		_progress = (_i / _filesCount); // ready-only
		__crystal_trace($"Scanning: [{_progress * 100}%] {_dirName} | {_fileName}", 2);
		if (directory_exists(_path)) {
			if (_searchSubdir) __crystal_directory_get_contents(_path, _contentsArray, _extension, _searchFiles, _searchFolders, _searchSubdir); // recursively search directories
			if (_searchFolders) _data = _path;
		} else {
			if (_searchFiles) _data = _path;
		}
