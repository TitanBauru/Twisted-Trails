...
function PPFX_Curve(size=512, mirror_x=false) constructor {
	...
	/// @method LoadAnimCurve(curve_asset, x_channel, y_channel, z_channel, w_channel)
	...
	/// @ignore
	/// @method ResetChannel(default_curve_asset)
	...
	/// @method LoadSprite(sprite, subimg)
	...
	/// @method LoadFile(file_path, secure)
	...
	/// @method SaveFile(file_path, secure, description)
	...
	/// @ignore
	/// @method DebugDraw(x1, y1, x2, y2, cursor_x, cursor_y, upper_range, lower_range, precision, alpha)
	static DebugDraw = function(x1, y1, x2, y2, cursor_x, cursor_y, upper_range=1, lower_range=0, precision=10, alpha=0) {
		...
		// no graph available
		if (!surface_exists(__debug_surf)) {
			var _spr_height = sprite_get_height(__ppf_sprDebugIcon);
			var _icon_x = x1+_width/2;
			var _icon_y = y1+_height/2;
			draw_sprite(__ppf_sprDebugIcon, 0, _icon_x, _icon_y-_spr_height/2);
			draw_set_color(c_white);
			draw_set_halign(fa_center);
			draw_set_valign(fa_top);
