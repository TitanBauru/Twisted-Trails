...
function Crystal_Renderer() constructor {
	...
	static __emptyFunction = function() {
		__crystal_trace("Renderer is destroyed, can't perform actions: addShadowCaster, addMaterial, addMaterialLayer, addRenderable, etc", 1);
	}
	...
	static __addMaterial = function(_material) {
		// sprites (if any)
		if (_material.normalSprite != undefined) ds_list_insert(__matNormalSprites, 0, _material); // move __matNormalSprites to __renderPass?! (investigar como acessar de forma eficiente)
	...
	/// @ignore
	static __addRenderable = function(_pass, _function) {
		ds_list_add(__renderPass[_pass].renderables, _function);
	}
	...
	static __calculateVRAM = function() {
		...
		var i = 0, isize = __renderPassAmount, _pass = undefined, _surf = undefined, _width = undefined, _height = undefined;
		repeat(isize) {
	...
	/// This should only be called when you are 100% sure that you will no longer use the same renderer instance.
	/// @method Destroy()
	static Destroy = function() {
		...
		__addShadowCaster = __emptyFunction;
		__addMaterialLayer = __emptyFunction;
		__addMaterial = __emptyFunction;
		__addRenderable = __emptyFunction;
		__calculateVRAM = __emptyFunction;
	...
	/// If the thing is "Static", It regenerates at a certain distance (by default, long).
	...
	/// @method SetCullingSettings(dynamicsBorderSize, dynamicsMoveDistance, dynamicsAutoUpdateTimer, staticsBorderSize, staticsMoveDistance)
	/// @param {Real} dynamicsBorderSize Sets the distance from the camera's edge at which dynamic stuff will stop from rendering. Default is 200. Increase this if you notice some things disappearing sooner.
	/// @param {Real} dynamicsMoveDistance Distance the camera needs to move to update the culling of dynamic things. Default is 10. Useful to avoid constant updates every frame. If the camera teleports, this should work naturally too.
	/// @param {Real} dynamicsAutoUpdateTimer The time (in frames) that the culling state should update if the camera is idle (not moving). Dynamics only.
	/// @param {Real} staticsBorderSize Sets the distance from the camera's edge at which static stuff will be regenerated and culled. Default is 2500. Increase this if you notice some things disappearing sooner.
	/// @param {Real} staticsdynamicsMoveDistance Distance the camera needs to move to update the culling of static things. Default is 250. Useful to avoid constant updates every frame. If the camera teleports, this should work naturally too.
	static SetCullingSettings = function(_dynamicsBorderSize=200, _dynamicsMoveDistance=10, _dynamicsAutoUpdateTimer=60, _staticsBorderSize=2500, _staticsdynamicsMoveDistance=250) {
		__cullingDynamicsViewBorderSize = _dynamicsBorderSize;
		__cullingDynamicsViewMoveDistance = _dynamicsMoveDistance;
		__cullingDynamicsAutoUpdateTimerBase = _dynamicsAutoUpdateTimer;
		__cullingStaticsViewBorderSize = _staticsBorderSize;
		__cullingStaticsViewMoveDistance = _staticsdynamicsMoveDistance;
		return self;
	}
	...
	/// @desc Enable or disable vertex shadow generation, even if you have created shadows. Disabling and then re-enabling this setting will cause the shadows to regenerate.
	/// This feature can be useful for your game performance options.
	/// @method SetVertexShadowsEnable(enabled)
	/// @param {Bool} enabled Defines if the renderer is generating vertex shadows. Use -1 to toggle.
	static SetVertexShadowsEnable = function(_enabled=-1) {
		if (_enabled == -1) {
			__isVertexShadowsEnabled = !__isVertexShadowsEnabled;
		} else {
			__isVertexShadowsEnabled = _enabled;
		}
		__vbuffStaticRebuild = true;
		return self;
	}
	...
	static SetHDREnable = function(_enabled=-1) {
		...
		repeat(lsize) {
			__matEmissiveLayers[| l].__cleanSurface();
			++l;
		}
		l = 0; lsize = ds_list_size(__matLightLayers);
		repeat(lsize) {
			__matLightLayers[| l].__cleanSurface();
			++l;
		}
		l = 0; lsize = ds_list_size(__matCombineLayers);
		repeat(lsize) {
			__matCombineLayers[| l].__cleanSurface();
			++l;
		}
		return self;
	}
	...
	/// @param {Enum.CRYSTAL_LIGHTS_BLENDMODE} blendMode Blendmode to blend lights. Example: CRYSTAL_LIGHTS_BLENDMODE.MULTIPLY.
	static SetLightsBlendmode = function(_blendMode=CRYSTAL_LIGHTS_BLENDMODE.MULTIPLY) {
		__lightsBlendMode = _blendMode;
		return self;
	}
	
	/// @desc Sets intensity of all lights.
	/// @param {Real} intensity Lights intensity. From 0 to infinity. 1 is fully lit (default).
	...
	/// @desc Sets intensity of all lights. Works like .SetLightsIntensity() but is not affected by any external factors (like Day & Night Cycle).
	/// @param {Real} intensity Lights intensity. From 0 to infinity. 1 is default (untouched).
	/// @method SetLightsCompensation(intensity)
	static SetLightsCompensation = function(_intensity) {
		__lightsCompensation = _intensity;
		return self;
	}
	...
	/// @desc Gets the current lights compensation intensity.
	/// @method GetLightsCompensation()
	static GetLightsCompensation = function() {
		return __lightsCompensation;
	}
		...
		if (__isRenderEnabled) {
									...
									repeat(_maskSpritesAmount) {
										with(_maskSpritesArray[| i]) {
											...
											if (__destroyed) {
												...
												// draw sprite
												if (enabled && !__cull && maskSprite != undefined) {
													gpu_set_depth(depth);
													if (isBitmap) {
									...
									repeat(_layersAmount) {
										with(_layersArray[| i]) {
											...
											if (__destroyed) {
												...
												if (__isRenderEnabled && surface_exists(__surface)) {
													// The effect will usually replace the shader, so it is expected that you will need to repeat the original shader code
													shader_set_uniform_f(_emission_u_intensity, max(emission, 0));
													gpu_set_depth(depth);
													if (__layerEffect == undefined) {
														draw_surface_stretched(__surface, 0, 0, _surfaceW, _surfaceH);
													} else {
										...
										with(_spritesArray[| i]) {
											...
											if (__destroyed) {
												...
												if (enabled && !__cull && emissiveSprite != undefined) {
													...
													gpu_set_depth(depth);
													if (isBitmap) {
														draw_sprite_ext(emissiveSprite, emissiveSpriteSubimg, x, y, xScale, yScale, angle, emissionColor, emissiveIntensity);
													} else {
														draw_skeleton_time(emissiveSprite, animName, skinName, animTime, x, y, xScale, yScale, angle, emissionColor, emissiveIntensity);
													}
												}
												++i;
											}
				...
				if (__lightsIntensity > 0) {
					...
					surface_set_target(_pass.surface, _surface);
						...
						#region Shared Uniforms + Blendmodes
							// Shadows
							static _u_shadowBasicParams = shader_get_uniform(__cle_shVertShadowBasic, "u_params"); // shadowScattering, shadowDepthOffset
							static _u_shadowBasicParams2 = shader_get_uniform(__cle_shVertShadowBasic, "u_params2"); // 2d light position
							static _u_shadowSmoothParams = shader_get_uniform(__cle_shVertShadowSmooth, "u_params"); // shadowPenumbra, shadowUmbra, shadowScattering, shadowDepthOffset
							static _u_shadowSmoothParams2 = shader_get_uniform(__cle_shVertShadowSmooth, "u_params2"); // 2d light position, lightPenetration
							
							static _u_dirShadowBasicParams = shader_get_uniform(__cle_shVertDirShadowBasic, "u_params"); // shadowScattering, shadowDepthOffset
							static _u_dirShadowBasicParams2 = shader_get_uniform(__cle_shVertDirShadowBasic, "u_params2"); // xDir, yDir
							static _u_dirShadowSmoothParams = shader_get_uniform(__cle_shVertDirShadowSmooth, "u_params"); // shadowPenumbra, shadowUmbra, shadowScattering, shadowDepthOffset
							static _u_dirShadowSmoothParams2 = shader_get_uniform(__cle_shVertDirShadowSmooth, "u_params2"); // xDir, yDir, lightPenetration
							// Blend modes
						...
						#region >> Directional/Sun Lights
							...
							with(__cle_objDirectLight) {
								...
								if (castShadows) {
									...
									if (shadowShaderType == LIGHT_SHADOW_SHADER_BASIC) {
										shader_set(__cle_shVertDirShadowBasic);
										shader_set_uniform_f(_u_dirShadowBasicParams, shadowScattering, shadowDepthOffset);
										shader_set_uniform_f(_u_dirShadowBasicParams2, dcos(angle), -dsin(angle));
									} else {
										shader_set(__cle_shVertDirShadowSmooth);
										shader_set_uniform_f(_u_dirShadowSmoothParams, shadowPenumbra/100, shadowUmbra, shadowScattering, shadowDepthOffset);
										shader_set_uniform_f(_u_dirShadowSmoothParams2, dcos(angle), -dsin(angle), max(penetration*10, CLE_CFG_EPSILON));
									}
						...
						#region >> Shape Lights
							...
							with(__cle_objShapeLight) {
								...
								if (castShadows) {
									...
									gpu_set_cullmode(selfShadows ? cull_noculling : cull_counterclockwise);
									if (shadowShaderType == LIGHT_SHADOW_SHADER_BASIC) {
										shader_set(__cle_shVertShadowBasic);
										shader_set_uniform_f(_u_shadowBasicParams, shadowScattering, shadowDepthOffset);
										shader_set_uniform_f(_u_shadowBasicParams2, x, y);
									} else {
										shader_set(__cle_shVertShadowSmooth);
										shader_set_uniform_f(_u_shadowSmoothParams, shadowPenumbra, shadowUmbra, shadowScattering, shadowDepthOffset);
										shader_set_uniform_f(_u_shadowSmoothParams2, x, y, max(penetration, CLE_CFG_EPSILON));
									}
									vertex_submit(_staticVbuff,  pr_trianglelist, -1);
									vertex_submit(_dynamicVbuff, pr_trianglelist, -1);
									gpu_set_cullmode(cull_counterclockwise);
			...
			#region Combine: Draw surfaces to a final surface
				#region uniforms
					...
					static __uniRenderLightsCompensation = shader_get_uniform(__cle_shDeferredRender, "u_lightsCompensation");
				...
				surface_set_target(_pass.surface);
					...
					if (__deferredFunction == undefined) {
						shader_set(__cle_shDeferredRender);
							...
							shader_set_uniform_f(__uniRenderLightsCompensation, __lightsCompensation);
							// dithering
	...
	static Draw = function(_x, _y, _w, _h) {
		...
		if (__isDrawEnabled) {
			...
			if (surface_exists(__finalRenderSurf)) {
				...
				if (surface_exists(__sourceSurface)) draw_surface_stretched(__sourceSurface, _x, _y, _w, _h);
			}
			gpu_pop_state();
		}
	}
...
/// Make sure a Crystal_Renderer exists before calling this function.
/// 
...
function crystal_pass_submit(_pass, _function) {
	...
	global.__CrystalCurrentRenderer.__addRenderable(_pass, _function);
}
